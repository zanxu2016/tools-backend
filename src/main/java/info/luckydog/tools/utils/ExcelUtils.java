...
public class ExcelUtils {
    ...
    public static final Map<Double, Product> DATA_MAP = new HashMap<>();
     ...
     */
    public static void readExcel(String filepath, Integer sheetNo)
...
//    }
    ...
    private static void reduceData(Sheet sheet) {

        if (sheet != null) {
            int rowNos = sheet.getLastRowNum();// 得到excel的总记录条数
            for (int i = 3; i <= rowNos; i++) {// 遍历行
                Row row = sheet.getRow(i);
                if (row == null || row.getCell(1) == null || "".equals(String.valueOf(row.getCell(1).getNumericCellValue()))) {
                    return;
                }
                // 取品种和数量，放入 map 中
                int noCellIndex = 1;
                int nameCellIndex = 2;
                int weightCellIndex = 4;
                int amountCellIndex = 6;
                Cell noCell = row.getCell(noCellIndex);
                Cell nameCell = row.getCell(nameCellIndex);
                Cell weightCell = row.getCell(weightCellIndex);
                Cell amountCell = row.getCell(amountCellIndex);
                if (noCell != null && nameCell != null && weightCell != null && amountCell != null) {
                    Double no = noCell.getNumericCellValue();
                    String name = nameCell.getStringCellValue();
                    System.out.println("sheetName:" + sheet.getSheetName() + ", row:" + i + ", name:" + name);
                    try {
                        Double weight = weightCell.getNumericCellValue();
                        Double amount = amountCell.getNumericCellValue();
                        if (DATA_MAP.containsKey(no)) {
                            weight += DATA_MAP.get(no).getWeight();
                            amount += DATA_MAP.get(no).getAmount();

                        }
                        DATA_MAP.put(no, new Product(no, name, weight, amount));
                    } catch (IllegalStateException e) {
                        System.out.println("获取数据异常：" + "sheetName:" + sheet.getSheetName() + ", row:" + i + ", name:" + name);
                        continue;
                    }
                }
            }
...
//    }
    public static boolean writeExcel(String filepath, String sheetName, List<String> titles,
                                     Map<Double, Product> values) throws IOException {
        boolean success = false;
        OutputStream outputStream = null;
        if (StringUtils.isBlank(filepath)) {
            throw new IllegalArgumentException("文件路径不能为空");
...
//                cell.setCellStyle(styles.get("header"));
            ...
            // 写入正文
            Set<Map.Entry<Double, Product>> entrySet = values.entrySet();
            // 行号
            int index = 1;
            for (Map.Entry<Double, Product> entry : entrySet) {
                row = sheet.createRow(index);

                // 在指定序号处创建cell
                Cell noCell = row.createCell(0);
                noCell.setCellValue(entry.getKey());

                Cell nameCell = row.createCell(1);
                nameCell.setCellValue(entry.getValue().getName());

                Cell weightCell = row.createCell(2);
                weightCell.setCellValue(entry.getValue().getWeight());

                Cell amountCell = row.createCell(3);
                amountCell.setCellValue(entry.getValue().getAmount());
                // 设置cell的样式
//                if (index % 2 == 1) {
//                    kindCell.setCellStyle(styles.get("cellA"));
//                } else {
//                    kindCell.setCellStyle(styles.get("cellB"));
//                }
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<String> titleList = Arrays.asList("编号", "品种", "数量", "金额");
        writeExcel(fileNameWrite, "汇总", titleList, DATA_MAP);
    }
}
